<?php
    ...
    $query_data_pelanggan = mysqli_query($koneksi, "SELECT * FROM pelanggan WHERE id_pelanggan='$id_pelanggan'");
    $data_pelanggan = mysqli_fetch_array($query_data_pelanggan);
    $id_pelanggan_tagihan = $data_pelanggan['id'];

    if($data_pelanggan == null)
    {
        $_SESSION['pamrh_flash_success'] = "Data tidak ditemukan selihakan mencoba lagi";
        header("location: scan.php");
    }
    
    $query_tagihan = mysqli_query($koneksi, "SELECT * FROM tagihan WHERE id_pelanggan = '$id_pelanggan_tagihan' AND status!='lunas'");
    $query_tagihan_lunas = mysqli_query($koneksi, "SELECT * FROM tagihan WHERE id_pelanggan = '$id_pelanggan_tagihan' AND status='lunas'");
    ...
    <div class="container-fluid p-3">
      <div class="row justify-content-center">
        <div class="col-12 wow fadeInUp">
          <h1 class="mb-3">Tagihan Belum Lunas</h1>
        ...
        <div class="col-12 mt-4 wow fadeInUp">
          <h1 class="mb-3">Tagihan Lunas</h1>
          ...
          <div class="text-lg">
            <table id="data_table" class="d-none d-md-block table table-striped table-bordered table-responsive" style="width:100%">
              ...
              <tbody>
                  <?php
                  foreach ($query_tagihan_lunas as $c) {
                  ?>
                  <tr>
                      <td><?php echo date('F Y', strtotime($c['tgl_cek'])); ?></td>
                      <td><?php echo date('d F Y', strtotime($c['tgl_cek'])); ?></td>
                      <td><?php echo $c['id_tagihan']; ?></td>
                      <td><?php echo $c['awal'].' - '.$c['akhir']; ?></td>
                      <td><?php echo $c['pemakaian']; ?></td>
                      <td><?php echo number_format($c['tagihan'],0,',','.'); ?></td>
                      <td class="text-capitalize"><?php echo $c['status']; ?></td>
                      <td><?php echo date('d F Y', strtotime($c['tgl_bayar'])); ?></td>
                      <td>
                        <?php if($c['file'] != null)
                        {
                        ?>
                        <a class="example-image-link" href="../assets/img/pembayaran/<?php echo $c['file']; ?>" data-lightbox="example-set"><img class="example-image" style="width: 100px" src="../assets/img/pembayaran/<?php echo $c['file']; ?>" alt=""/></a>
                        <?php
                        }
                        ?>
                      </td>
                      <td>
                        <a class="example-image-link" href="../assets/img/meteran/<?php echo $c['foto_meteran']; ?>" data-lightbox="example-set"><img class="example-image" style="width: 100px" src="../assets/img/meteran/<?php echo $c['foto_meteran']; ?>" alt=""/></a>
                      </td>
                      <td>
                        <?php
                        if($c['status'] == 'butuh bayar')
                        {
                        ?>
                        <a  href="tagihan_bayar.php?id=<?php echo $c['id']; ?>" class="btn btn-sm pt-1 pb-1 pl-3 pr-3 btn-success"><span class="iconify" data-icon="akar-icons:money"></span></a>
                        <?php
                        }
                        if($c['status'] == 'butuh konfirmasi') {
                        ?>
                        <a  href="tagihan_bayar.php?id=<?php echo $c['id']; ?>" class="btn btn-sm pt-1 pb-1 pl-3 pr-3 btn-warning"><span class="iconify" data-icon="akar-icons:edit"></span></a>
                        <?php
                        }
                        ?>
                      </td>
            ...
            <div class="d-block d-md-none m-0 p-2">
              <?php
              foreach ($query_tagihan_lunas as $d) {
              ?>
              <div class="card text-center col-12 mt-2 mb-2 ml-0 mr-0">
                <div class="card-header">
                  <?php echo $d['id_tagihan']; ?>
                </div>
                <div class="card-body">
                  <a class="example-image-link" href="../assets/img/pembayaran/<?php echo $d['file']; ?>" data-lightbox="example-set"><img class="example-image col-5" src="../assets/img/pembayaran/<?php echo $d['file']; ?>" alt=""/></a>
                  <a class="example-image-link" href="../assets/img/meteran/<?php echo $d['foto_meteran']; ?>" data-lightbox="example-set"><img class="example-image col-5" src="../assets/img/meteran/<?php echo $d['foto_meteran']; ?>" alt=""/></a>
                  <table class="table table-sm">
                    <tbody>
                      <tr>
                        <th class="text-left" scope="row">Tagihan Bulan</th>
                        <td class="text-right"><?php echo date('F Y', strtotime($d['tgl_cek'])); ?></td>
                      </tr>
                      <tr>
                        <th class="text-left" scope="row">Meteran</th>
                        <td class="text-right"><?php echo $d['awal'].'-'.$d['akhir']; ?></td>
                      </tr>
                      <tr>
                        <th class="text-left" scope="row">Pemakaian</th>
                        <td class="text-right"><?php echo $d['pemakaian']; ?></td>
                      </tr>
                      <tr>
                        <th class="text-left" scope="row">Tagihan</th>
                        <td class="text-right"><?php echo 'Rp '.number_format($d['tagihan'],0,',','.'); ?></td>
                      </tr>
                      <tr>
                        <th class="text-left" scope="row">Status</th>
                        <td class="text-right text-capitalize"><?php echo $d['status']; ?></td>
                      </tr>
                    </tbody>
                  </table>
                  <?php
                    if($d['status'] == 'butuh bayar')
                  {
                  ?>
                  <a href="tagihan_bayar.php?id=<?php echo $c['id']; ?>" class="btn btn-sm pt-1 pb-1 pl-3 pr-3 btn-success"><span class="iconify" data-icon="akar-icons:money"></span></a>
                  <?php
                  }
                  if($d['status'] == 'butuh konfirmasi') {
                  ?>
                  <a href="tagihan_edit.php?id=<?php echo $c['id']; ?>" class="btn btn-sm pt-1 pb-1 pl-3 pr-3 btn-warning"><span class="iconify" data-icon="akar-icons:edit"></span></a>
                  <?php
                  }
                  ?>
                </div>
                <div class="card-footer text-muted">
                  <?php echo date('d F Y', strtotime($d['tgl_cek'])); ?>
                </div>
              </div>
              <?php
              }
